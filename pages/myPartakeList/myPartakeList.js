...
Page({
  ...
  onLoad: function (options) {
    ...
    if (myPartakeList) this.setData({ myPartakeList: wx.getStorageSync('myPartakeList') });
      ...
      let url = new URL('https', serverAddr).path('subjects').param('page', this.pageNum).param('queryType', 'participant');
      wx.request({
        url: url.toString(),
        method: 'GET',
        header: {
  ...
  onShow: function () {
    if (this.neverShow) this.neverShow = false;
      ...
      let url = new URL('https', serverAddr).path('subjects').param('page', 1).param('queryType', 'participant');
      wx.request({
        url: url.toString(),
        method: 'GET',
        header: {
  ...
  onReachBottom: function () {
    ...
    // 判断还有无数据
    console.log("触底刷新hasNextPage:", wx.getStorageSync('myPartakePageData').hasNextPage)
    this.hasNextPage = wx.getStorageSync('myPartakePageData').hasNextPage;
        ...
        success: ({ data: result, statusCode }) => {
          ...
          switch (statusCode) {
            case 200:
              // 缓存页面数据，包括arrSize、array、pageNum
              wx.setStorageSync('myPartakePageData', result);
              console.log("onReachBottom运行了", wx.getStorageSync('myPartakePageData'))
