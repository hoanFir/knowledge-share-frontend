...
class CommentService {
   ...
   * 获得评论
   */
  fetchComments(ksId, pageNum, callback) {

    let url = new URL('http', serverAddr).path('subjects/' + ksId + '/comments').param('page', pageNum);
    wx.request({
      url: url.toString(),
      method: 'GET',
  ...
  commentActivity(ksId, ComContent, callback) {
    ...
    let url = new URL('http', serverAddr).path('subjects/' + ksId + '/comments').param('content', ComContent);
    wx.request({
      url: url.toString(),
      method: 'POST',
      header: {
  ...
  updateMoreInfo(data, ksId, kcId, callback) {
    ...
    let url = new URL('http', serverAddr).path('subjects/' + ksId + 'comments/' + kcId);
    wx.request({
      url: url.toString(),
      method: 'PUT',
      header: {
