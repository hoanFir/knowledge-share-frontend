...
class UserService {
  ...
  validate(code, userdata, callback) {
    ...
    let url = new URL('https', serverAddr).path('tokens/' + code);
    wx.request({
      url: url.toString(),
      method: 'POST',
      ...
      success: ({ data: result, statusCode }) => {
        ...
        switch (statusCode) {
          case 200:
            wx.setStorageSync('sid', result.data.accessToken);
            wx.setStorageSync('userDetail', result.data.ksUser);
            callback();
            break;
  ...
  getActivityUserDetail(kuId, callback) {
    ...
    let url = new URL('https', serverAddr).path('users/' + kuId);
    wx.request({
      url: url.toString(),
      method: 'GET',
      header: {
  ...
  updateBasicInfo(data, callback) {
    ...
    let url = new URL('https', serverAddr).path('users' + '/' + data.kuId);
    wx.request({
      url: url.toString(),
      method: 'PUT',
      header: {
